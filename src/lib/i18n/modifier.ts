import { DOCUMENT_REGEX, LOCALE_PATHNAME_REGEX, SUPPORTED_LOCALE } from '$lib/i18n/constants';

export const addLocaleToResponseBody = (request, response) => {
	if (DOCUMENT_REGEX.test(request.path) && response.body) {
		response.body = response.body.replace('%svelte.lang%', request.context.locale);
	}
};
